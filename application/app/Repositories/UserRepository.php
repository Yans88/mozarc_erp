...
class UserRepository {
    ...
    public function getByEmployeeId($employee_id){
        ...
        $users->where('employee_id', $employee_id);
    ...
    public function search($id = '') {
		...
		if (request()->filled('status_employee')) {
            ...
            $users->where('status_employee', request('status_employee'));
        ...
        if (request()->filled('clientid')) {
		 ...
		 if (request()->filled('notin') && !empty(array_filter(request('list_employee')))) {

            $users->whereNotIn('id', request('list_employee'));
    ...
    public function create($password = '', $returning = 'id') {
		...
		$user->name_pronouciation = request('name_pronouciation');
		$user->department_name = request('department_name');
		$user->company_email = request('company_email');		
		$user->date_birth = (!request()->filled('date_birth')) ? NULL : request('date_birth');
		$user->gender = request('gender');
		$user->status_employee = request('status_employee');
		$user->joined_date = (!request()->filled('joined_date')) ? NULL : request('joined_date');
		$user->resigned_date = (!request()->filled('resigned_date')) ? NULL : request('resigned_date');
		$user->resigned_reason = request('resigned_reason');
		...
		$user->swift_iban = request('swift_iban');
