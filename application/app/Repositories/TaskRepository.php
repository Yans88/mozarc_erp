...
class TaskRepository {
    ...
    public function search($id = '', $data = []) {
		...
		$tasks->leftJoin('leads', 'leads.lead_id', '=', 'tasks.task_leadid');
		
        $tasks->leftJoin('roles', 'roles.role_id', '=', 'tasks.task_departmentid');
		...
		if (request('filter_project_type') == 'all' && !request()->filled('filter_my_tasks')) {
			
			$tasks->Where(function ($query) {
				$query->Where(function ($q) {
                    $q->where('task_projectid', '>', 0);
					$q->orWhere('task_leadid', '>', 0);
                });
				
				$query->orWhere(function ($q) {
                    $q->Where('task_private', '=', 1);
					$q->Where('task_creatorid', '=', auth()->id());
        ...
        if ($data['apply_filters']) {
            ...
            if (request()->filled('task_leadid')) {

                $tasks->where('task_leadid', request('task_leadid'));
            ...
            if (request()->filled('filter_my_tasks')) {
				...
				// new
				$tasks->where('task_private', '=', 1);
                $tasks->where('task_creatorid', '=', auth()->id());
    ...
    public function create($position = '') {
		...
		$task->task_leadid = request('task_leadid');
		
		$task->task_private = request('show_task_private') ? 1 : 0;
		
		$task->task_departmentid = request('task_departmentid');
