    ...
    public function search($id = '', $data = []) {
        ...
        if ($data['apply_filters']) {
            ...
            if (request()->filled('filter_subscription_date_started_start')) {
                $subscriptions->where('subscription_date_started', '>=', request('filter_subscription_date_started_start'));
            }

            //filter: subscription date started (end)
            if (request()->filled('filter_subscription_date_started_end')) {
                $subscriptions->where('subscription_date_started', '<=', request('filter_subscription_date_started_end'));
            }

            //filter: subscription date ended (start)
            if (request()->filled('filter_subscription_date_ended_start')) {
                $subscriptions->where('subscription_date_ended', '>=', request('filter_subscription_date_ended_start'));
            }

            //filter: subscription date ended (end)
            if (request()->filled('filter_subscription_date_ended_end')) {
                $subscriptions->where('subscription_date_ended', '<=', request('filter_subscription_date_ended_end'));
            }

            //filter: subscription date renewed (start)
            if (request()->filled('filter_subscription_date_renewed_start')) {
                $subscriptions->where('subscription_date_renewed', '>=', request('filter_subscription_date_renewed_start'));
            }

            //filter: subscription date renewed (end)
            if (request()->filled('filter_subscription_date_renewed_end')) {
                $subscriptions->where('subscription_date_renewed', '<=', request('filter_subscription_date_renewed_end'));
            }
