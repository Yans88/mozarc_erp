...
class Index {
    ...
    public function handle($request, Closure $next) {
        ...
        }				Log::error(request('fileresource_type'));		if (request('fileresource_type') == 'employee') {						Log::error(request('fileresource_type'));            //only this client            // request()->merge([                // 'filter_file_clientid' => request('fileresource_id'),            // ]);            if (auth()->user()->is_team) {                if (auth()->user()->role->role_clients >= 1) {                    return $next($request);                }            }        }
    ...
    private function fronteEnd() {
        ...
        //client files (not client project files)
        if (request('fileresource_type') == 'client' && request()->filled('fileresource_id') || request('fileresource_type') == 'employee') {
